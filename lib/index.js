...
function getNetatmoIndoorMesures(callback) {
    getNetatmoLastMesures(NETATMO_INDOOR_DATA.join(","), null, callback);
}

function getNetatmoOutdoorMesures(callback) {
    getNetatmoLastMesures(NETATMO_OUTDOOR_DATA.join(","), NETATMO_MODULE_ID, callback);
}

function getNetatmoLastMesures(data, moduleId, callback) {
...
function syncMesures(options, callback) {
    ...
    _.each(options.netatmoData, function(netatmoDataName, dataIndex) {
        ...
        _.each(options.mesures, function(values, timestamp) {
            ...
            if (!options.lastMoments[dataIndex] ||  mesureMoment.isAfter(options.lastMoments[dataIndex])) {
