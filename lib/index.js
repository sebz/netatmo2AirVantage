...
var NETATMO_BASE_URL = "https://api.netatmo.net",
    ...
    NETATMO_INDOOR_MODULE2_ID = netatmoConf.indoorModule2Id;
...
var NETATMO_INDOOR_DATA = [
    ...
    AIRVANTAGE_INDOOR2_DATA = [
        "home.indoor.upstairs.temperature",
        "home.indoor.upstairs.co2",
        "home.indoor.upstairs.humidity",
    ],
...
function checkAVIndoor2Data(callback) {
    avIndoor2LastMoments = [];
    console.log("# ---- INDOOR 2----");
    checkAirVantage({
        dataId: AIRVANTAGE_INDOOR2_DATA.join(","),
        lastMoments: avIndoor2LastMoments
    }, callback);
}
        ...
        if (lastData) {
            _.each(lastData, function(values, dataPath) {
                if (values) {
                    ...
                    console.log("# Last synchronized", "'" + dataPath + "'", "Never");
                }
            });
        }
        callback(null);
...
function getNetatmoIndoor2Mesures(callback) {
    getNetatmoLastMesures(NETATMO_INDOOR2_DATA.join(","), avIndoor2LastMoments, NETATMO_INDOOR_MODULE2_ID, callback);
}
...
function getOldestMoment(lastMoments) {
    ...
    if (!lastMoments.length) {
        oldestMoment = moment().subtract(3, "month");
    }
...
function syncIndoor2Mesures(mesures, callback) {
    syncMesures({
        mesures: mesures,
        netatmoData: NETATMO_INDOOR2_DATA,
        airvantageData: AIRVANTAGE_INDOOR2_DATA,
        lastMoments: avIndoor2LastMoments
    }, callback);
}
...
// Check last indoor2 data on AirVantage
tasks.push(checkAVIndoor2Data);
// Get last indoor2 mesures from Netatmo
tasks.push(getNetatmoIndoor2Mesures);
tasks.push(syncIndoor2Mesures);
